...
export default function Meridian3DPanel({drafts,selectedPointCode,studyCommand,onStudyAction,onOverlayChange,onFocus}:Props){
  ...
  return <>
    <Button variant="ghost" className={'meridian3d-launch '+(enabled?'active':'')} ref={node=>{launchDrag.ref.current=node}} onClick={()=>{if(open){setOpen(false);return}setOpen(true);setEnabled(true);prepareMeridianView(activeMeridian)}} aria-pressed={enabled} aria-expanded={open} aria-label="Mở mô hình kinh lạc 3D" data-meridian3d-launch="true">
    ...
    {open&&<aside data-floating-menu="meridians" className="meridian3d-panel glass" ref={node=>{panelDrag.ref.current=node}} data-meridian3d-panel="true" aria-label="Mô hình kinh lạc và huyệt vị 3D">
      ...
      <section className="acupuncture-sim" data-acupuncture-simulator="true" aria-label="Mô phỏng châm cứu 3D">
        ...
        {!selectedRecord||!needleAnchor?<p role="status">Chọn một huyệt có vị trí 3D để bật mô phỏng.</p>:<>
          ...
          <p>Động tác đang chọn: {needleAction==='insert'?'chuyển động dọc theo trục kim':needleAction==='twist'?'xoay trục kim': 'chuyển động nâng và đẩy'}. Mô hình kèm cánh tay và bàn tay 3D minh họa cách kẹp kim; đây không phải hướng dẫn kỹ thuật lâm sàng.</p>
