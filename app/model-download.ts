...
export async function decodeModelResponse(response:Response,expectedBytes:number,compressed:boolean):Promise<ArrayBuffer>{
 if(!response.ok)throw new Error('Không tải được tệp giải phẫu.');
 ...
 if(buffer.byteLength!==expectedBytes)throw new Error('Tệp giải phẫu tải chưa đầy đủ. Vui lòng tải lại mô hình.');
 return buffer;
}
