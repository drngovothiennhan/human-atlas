...
try{
  ...
  const surfaceRuntime=await evaluate("(()=>{const d=document.querySelector('canvas')?.dataset||{};return{structure:d.surfacePrimaryStructure,count:Number(d.surfaceStructureCount||0),face:d.surfaceFacePolicy,loadPolicy:d.anatomyLoadPolicy,initialChunks:Number(d.initialAnatomyChunkCount||0),loading:Number(d.anatomyLoadingChunks||0)}})()");
  if(surfaceRuntime.structure!=='FMA7163'||surfaceRuntime.count!==1||surfaceRuntime.face!=='front-side-only'||surfaceRuntime.loadPolicy!=='visible-chunks-on-demand'||surfaceRuntime.initialChunks!==1)throw new Error('Single-skin/lazy-load runtime assertion failed: '+JSON.stringify(surfaceRuntime));
  await waitFor(()=>evaluate("document.querySelector('canvas')?.dataset.anatomyLoadingChunks==='0'"),{timeout:90000,label:'surface lazy chunks settled'});
  console.log('SMOKE_SINGLE_SKIN_LAZY_LOAD_PASS '+JSON.stringify(surfaceRuntime));
  ...
  const meridianVisualMetrics=await evaluate("(()=>{const d=document.querySelector('canvas')?.dataset||{};return{outer:Number(d.meridianLineOuterRadius),core:Number(d.meridianLineCoreRadius),opacity:Number(d.meridianLineCoreOpacity),transparent:d.meridianLineTransparent==='true',depthOcclusion:d.meridianDepthOcclusion==='anatomy-surface',point:Number(d.meridianPointMinRadius)}})()");
  if(!(meridianVisualMetrics.outer<=.0024&&meridianVisualMetrics.core<=.0017&&meridianVisualMetrics.opacity<1&&meridianVisualMetrics.transparent&&meridianVisualMetrics.depthOcclusion&&meridianVisualMetrics.point>meridianVisualMetrics.outer*2))throw new Error('Meridian visual hierarchy/occlusion assertion failed: '+JSON.stringify(meridianVisualMetrics));
  ...
  if(!await evaluate("(()=>{const b=[...document.querySelectorAll('[data-acupuncture-simulator=true] button')].find(x=>x.textContent.includes('Hiện kim mô phỏng'));if(!b)return false;b.click();return true})()"))throw new Error('Needle simulation toggle missing');
  await waitFor(()=>evaluate("document.querySelector('canvas')?.dataset.acupunctureSimulation==='ST-36'&&document.querySelector('canvas')?.dataset.acupunctureOperatorRig==='stylized-hand-arm'&&Number(document.querySelector('canvas')?.dataset.acupunctureHandPartCount)>=10"),{label:'3D practitioner hand and arm rig attaches to selected acupoint'});
  if(!await evaluate("(()=>{const b=[...document.querySelectorAll('[data-acupuncture-simulator=true] button')].find(x=>x.textContent.includes('Chạy mô phỏng động tác'));if(!b)return false;b.click();return true})()"))throw new Error('Needle action control missing');
  await waitFor(()=>evaluate("document.querySelector('canvas')?.dataset.acupunctureOperatorRig==='stylized-hand-arm'"),{label:'hand rig remains active with needle movement'});
  await evaluate("(()=>{const b=[...document.querySelectorAll('[data-acupuncture-simulator=true] button')].find(x=>x.textContent.includes('Tạm dừng động tác'));b?.click();return true})()");
  await evaluate("(()=>{const b=[...document.querySelectorAll('[data-acupuncture-simulator=true] button')].find(x=>x.textContent.includes('Tắt mô phỏng kim'));b?.click();return true})()");
  await waitFor(()=>evaluate("document.querySelector('canvas')?.dataset.acupunctureOperatorRig==='off'&&document.querySelector('canvas')?.dataset.acupunctureHandPartCount==='0'"),{label:'operator rig clears when needle simulation is disabled'});
